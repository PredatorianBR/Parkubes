  ...
  const renderCameraSettings = () => (
    <div className="grid grid-cols-1 gap-2">
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label
          htmlFor="cameraFollow"
          className="pixel-font text-[8px] text-gray-400 cursor-pointer"
        >
          CÂMERA SEGUIR
        </label>
      </div>
    ...
    >
      ...
      <div className="border-t border-white/10 pt-2">
        <div className="flex items-center gap-2 mb-2">
          ...
          <label
            htmlFor="debugMode"
            className={`pixel-font text-[8px] cursor-pointer ${debugMode ? 'text-red-400' : 'text-gray-500'}`}
          >
            MODO DEBUG
          </label>
        </div>
            ...
            {setShowAIPath && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  id="showAIPath"
                  checked={!!showAIPath}
                  onChange={(e) => setShowAIPath(e.target.checked)}
                  className="w-3 h-3 accent-cyan-500"
                />
                <label
                  htmlFor="showAIPath"
      ...
      {debugMode && gameState.status === GameStatus.PLAYING && (
        <div className="mt-3 border-t border-white/10 pt-2 flex flex-col gap-1.5">
          <h4 className="pixel-font text-[8px] text-red-400 mb-0.5">DEBUG CONTROLE</h4>
          <button
            onClick={() => handleRoundEnd?.(true)}
            className="pixel-font bg-green-700/60 hover:bg-green-600 text-white text-[8px] py-1.5 rounded transition-colors border border-green-500/30"
          >
            ENCERRAR (VITÓRIA)
          </button>
          <button
            onClick={() => handleRoundEnd?.(false)}
            className="pixel-font bg-red-900/60 hover:bg-red-800 text-white text-[8px] py-1.5 rounded transition-colors border border-red-500/30"
          >
            ENCERRAR (DERROTA)
          </button>
        </div>
      ...
      {gameState.status === GameStatus.PAUSED && (
        <div className="mt-4 flex flex-col gap-2">
          <button
            ...
            className="pixel-font bg-gray-700/50 hover:bg-gray-600 text-white text-[8px] py-2 rounded transition-colors"
          >
            RESPAWN
          </button>
          <button
            onClick={resetToMenu}
            className="pixel-font bg-red-900/40 hover:bg-red-800 text-white text-[8px] py-2 rounded transition-colors border border-red-500/30"
          >
