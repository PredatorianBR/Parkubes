...
export const DoorBlock: React.FC<{
  ...
  return (
    ...
    >
      ...
      <mesh
        position={[0, 1.1 - frameWidth / 2, frameZ]}
        receiveShadow
      ...
      <mesh position={[0, -0.025, 0.01]} userData={{ type: 'detail-fade' }}>
      ...
      {/* Handle */}
      <mesh position={[0.35, -0.1, 0.025]} userData={{ type: 'detail-fade' }}>
...
export const IndustrialDoorBlock: React.FC<{
  position: [number, number, number];
  rotation: [number, number, number];
}> = ({ position, rotation }) => {
  ...
  return (
    ...
    >
      ...
      <mesh
        position={[0, 1.5 - frameWidth / 2, frameZ]}
        receiveShadow
        userData={{ type: 'detail-fade' }}
      >
      ...
      <mesh position={[0, -0.025, 0.015]} userData={{ type: 'detail-fade' }}>
        <boxGeometry args={[1.8, 2.95, 0.03]} />
        <meshStandardMaterial color="#94a3b8" metalness={0.6} roughness={0.4} />
      </mesh>

      {[0.25, 0.75, 1.25, -0.25, -0.75, -1.25].map((y, i) => (
        <mesh key={i} position={[0, y - 0.2, 0.035]} userData={{ type: 'detail-fade' }}>
