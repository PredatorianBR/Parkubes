...
export const Ladder: React.FC<{
  ...
  const railMaterial = React.useMemo(
    () => new THREE.MeshStandardMaterial({ color: railColor, roughness: 0.9 }),
    [railColor],
  );
  const rungMaterial = React.useMemo(
    () => new THREE.MeshStandardMaterial({ color: rungColor, roughness: 0.85 }),
    [rungColor],
  );
  ...
  return (
    <group
      ...
      rotation={rotation}
      scale={[2, 2, 2]}
      userData={{ ignoreRaycast: true, type: 'ladder' }}
    >
      {/* Left Rail */}
      <mesh
        position={[-rungWidth / 2, 0, 0]}
        castShadow
        receiveShadow
        userData={{ type: 'ladder' }}
      >
