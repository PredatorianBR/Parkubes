...
export const RoofAC: React.FC<{ position: THREE.Vector3, scale: [number, number, number], color: string, rotation?: number, showGrid?: boolean }> = ({ position, scale, color, rotation = 0, showGrid = false }) => {
    // scale[0] = width, scale[1] = height, scale[2] = depth
    const w = scale[0];
    const h = scale[1];
    const d = scale[2];
