...
export const Chimney: React.FC<{
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]} userData={{ type: 'detail-fade' }}>
      ...
      {isIndustrial &&
        scale[1] > 1.5 &&
        !isIndustrial && ( // Keeping logic for specific cases if needed, but suppressed by !isIndustrial
          <group>
            <mesh
              position={[0, -0.5, -scale[2] / 2 - 0.05]}
              castShadow
              userData={{ type: 'detail-fade' }}
            >
