...
export const Character: React.FC<CharacterProps> = ({
    ...
    useFrame((state, delta) => {
        ...
        if (blinkPhase.current === 0) {
            ...
            if (p >= 1) { blinkPhase.current = 2; blinkTimer.current = 0; }
        ...
        if (isGrounded && !isMoving && !isRunning && !stunned && !isRolling && !isStumbling && !isCharging && !isHiding && !isClimbing && !isLadderSliding && !isNearLadder) {
            ...
            if (idleState.current === 0) { // Breathing/Resting
                if (idleTimer.current > 0.5 + Math.random() * 1.5) {
                    if (!isTiredBreathing.current) {
                        const roll = Math.random();
                        if (roll < 0.35) idleState.current = 1; // Look around
                        else if (roll < 0.65) idleState.current = 2; // Shift weight
            ...
            } else if (idleState.current === 3) { // Stretch (new!)
        ...
        if (isMoving && isGrounded && !stunned && !isRolling) {
            ...
            if (inWater) {
                ...
                // 6. Horizontal wiggle
                targetRotY = Math.sin(t) * (isRunning ? 0.10 : 0.07);
            ...
            if (isLadderMounting) {
                ...
                if (idleTimer.current > 1.5 + Math.random() * 2.0) {
                    const roll = Math.random();
                    if (roll < 0.5) idleState.current = 1; // Look around
        ...
        if (headMesh.current) {
            ...
            headMesh.current.position.y = THREE.MathUtils.lerp(headMesh.current.position.y, headTargetY, squashLerpSpeed);
            // Head independent rotation (Y = look direction, X = tilt)
            headMesh.current.rotation.y = THREE.MathUtils.lerp(headMesh.current.rotation.y, headRotYTarget, delta * 10);
        ...
        if (eyesMesh.current) {
            ...
            eyesMesh.current.position.y = THREE.MathUtils.lerp(eyesMesh.current.position.y, eyeTargetY, squashLerpSpeed);
            // Sync Eyes Rotation with Head
            eyesMesh.current.rotation.y = THREE.MathUtils.lerp(eyesMesh.current.rotation.y, headRotYTarget, delta * 10);
