...
export const updatePlayerPhysics = (
  ...
  const justPressedJump = aiInput
    ? aiInput.jump && !jumpPressedPrev.current
    : isJumpDown && !jumpPressedPrev.current;
  ...
  return {
    isRunning:
      !!((keys.current['shift'] && !aiInput) || isAnalogRunning || (aiInput && aiInput.run)) &&
      inputDir.lengthSq() > 0,
