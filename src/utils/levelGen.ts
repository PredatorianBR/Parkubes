...
export const findSpawnPos = (
  ...
  for (let k = 0; k < 1000; k++) {
    ...
    let minX = safePadding,
      maxX = size - safePadding;
    let minZ = safePadding,
      maxZ = size - safePadding;

    if (quadrant === 1) {
      minX = halfSize + centerMargin;
      maxZ = halfSize - centerMargin;
    } else if (quadrant === 2) {
      maxX = halfSize - centerMargin;
      maxZ = halfSize - centerMargin;
    } else if (quadrant === 3) {
      maxX = halfSize - centerMargin;
      minZ = halfSize + centerMargin;
    } else if (quadrant === 4) {
      minX = halfSize + centerMargin;
      minZ = halfSize + centerMargin;
    }
  ...
  if (quadrant) {
    ...
    if (quadrant === 1) {
      minX = halfSize + outerMargin;
      maxZ = halfSize - outerMargin;
    } else if (quadrant === 2) {
      maxX = halfSize - outerMargin;
      maxZ = halfSize - outerMargin;
    } else if (quadrant === 3) {
      maxX = halfSize - outerMargin;
      minZ = halfSize + outerMargin;
    } else if (quadrant === 4) {
      minX = halfSize + outerMargin;
      minZ = halfSize + outerMargin;
    }
...
export const generateCityLevel = (
  ...
  const bGrid: number[][] = Array(gridSize)
    .fill(null)
    .map(() => Array(gridSize).fill(0)); // Bridge Grid
  const wGrid: number[][] = Array(gridSize)
    .fill(null)
    .map(() => Array(gridSize).fill(0)); // Water Grid
  const sGrid: number[][] = Array(gridSize)
    .fill(null)
    .map(() => Array(gridSize).fill(0)); // Surface Grid (0: Grass, 1: Water, 2: Street, 3: Hard)

  // Type Grid remains Logic Resolution (1x1) for building placement logic
  const tGrid: number[][] = Array(size)
    .fill(null)
  ...
  if (hasRiver) {
    ...
    if (startEdge === 0) {
      cx = Math.floor(Math.random() * (size - 14)) + 7 - halfSize;
      cz = -halfSize;
    } else if (startEdge === 1) {
      cx = halfSize;
      cz = Math.floor(Math.random() * (size - 14)) + 7 - halfSize;
    } else if (startEdge === 2) {
      cx = Math.floor(Math.random() * (size - 14)) + 7 - halfSize;
      cz = halfSize;
    } else {
    ...
    let tz = 0;
    if (endEdge === 0) {
      tx = Math.floor(Math.random() * (size - 14)) + 7 - halfSize;
      tz = -halfSize - 2;
    } else if (endEdge === 1) {
      tx = halfSize + 2;
      tz = Math.floor(Math.random() * (size - 14)) + 7 - halfSize;
    } else if (endEdge === 2) {
      tx = Math.floor(Math.random() * (size - 14)) + 7 - halfSize;
      tz = halfSize + 2;
    } else {
    ...
    for (let i = 0; i < maxSteps; i++) {
      ...
      const width =
        i < 8 ? maxWidth : minWidth + Math.floor(Math.random() * (maxWidth - minWidth + 1));
      ...
      if (riverOrientation % 2 === 0) {
        // N-S flow (North-South or South-North)
        cz += dz;
        if (Math.random() > 0.7) cx += Math.random() > 0.5 ? 1 : -1;
        else if (Math.abs(tx - cx) > 1) cx += dx * 0.5;
      } else {
        // E-W flow (East-West or West-East)
        cx += dx;
  ...
  const placeBuilding = (
    ...
    bw: number,
    bd: number,
    forcedType?: 'farm' | 'ruins' | 'house' | 'factory' | 'highrise',
    categoryFilter?: 'farm' | 'building' | 'ruins',
  ): (() => void) | null => {
    ...
    if (forcedType) {
      ...
      if (categoryFilter === 'farm') {
        ...
        eligibleTypes = eligibleTargets.filter(
          (t) => t === 'house' || t === 'factory' || t === 'highrise',
        );
    ...
    let mask = Array(fillW)
      .fill(null)
      .map(() => Array(fillD).fill(true));
    ...
    if (fillW >= 6 && fillD >= 6 && Math.random() > 0.3) {
      ...
      if (startNode && trueCount >= 9 && !hasPinch) {
        ...
        const visited = Array(fillW)
          .fill(null)
          .map(() => Array(fillD).fill(false));
    ...
    if (type === 'highrise')
      baseColor = colors.highrise[Math.floor(Math.random() * colors.highrise.length)];
    if (type === 'factory')
      baseColor = colors.factory[Math.floor(Math.random() * colors.factory.length)];

    const assignedWindows: { pos: Position; rot: [number, number, number] }[] = [];
    const attachedChimneys: {
      pos: Position;
      scale: Position;
      color: string;
      smoke?: boolean;
    ...
    return () => {
            ...
            if (!isSolid(nx, nz)) {
              ...
              if (tx >= 0 && tx < size && tz >= 0 && tz < size && tGrid[tx][tz] === 0) {
                ...
                columns.push({
                  ...
                  dz: n.dz,
                  rot: n.rot,
                  rotVec: n.rotVec as [number, number, number],
                  minDist: minDist,
                  biasDist: biasDist,
                  facadeId: facadeId,
                });
      ...
      facadeMap.forEach((cols, id) => {
        ...
        groups.forEach((groupCols, groupIdx) => {
          facades.push({
            ...
            height: vH,
            occupancy: Array(groupCols.length)
              .fill(null)
              .map(() => Array(vH).fill(false)),
            hasDoor: false,
            hasWindow: false,
          });
        });
      ...
      for (const f of facades) {
        const isNearMapEdge = f.columns.some(
          (col) => Math.abs(col.worldX) > halfSize - 6 || Math.abs(col.worldZ) > halfSize - 6,
        );
        ...
        if (doorsPlacedCount < maxDoors) {
          ...
          for (const startX of possibleStarts) {
            ...
            for (let x = startX - pad; x <= startX + dW + (pad - 1); x++) {
              ...
              if (!canPlace) break;
              if (
                globalColumnOccupied.has(
                  `${f.columns[Math.max(0, Math.min(x, f.width - 1))].worldX},${f.columns[Math.max(0, Math.min(x, f.width - 1))].worldZ}`,
                )
              ) {
                canPlace = false;
                break;
            ...
            if (canPlace) {
              ...
              doorsPlacedCount++;
              if (dType === 'industrial') indDoorsCount++;
              else stdDoorsCount++;
          ...
          if (cX >= 1 && cX + cW <= f.width - 1) {
            ...
            for (let x = cX - 1; x <= cX + cW; x++) {
              for (let y = 0; y < f.height; y++) {
                if (x < 0 || x >= f.width || f.occupancy[x][y]) {
                  canPlace = false;
                  break;
                }
              }
      ...
      if (
        (doorsPlacedCount === 0 || (type === 'factory' && indDoorsCount === 0)) &&
        facades.length > 0
      ) {
        const isFactory = type === 'factory';
        // Try facades for a door
        for (const f of facades) {
          const dType: 'industrial' | 'standard' =
            isFactory && indDoorsCount === 0 ? 'industrial' : 'standard';
          const dW = dType === 'industrial' ? 4 : 2;
          const dH = dType === 'industrial' ? 6 : 4;
      ...
      for (const f of facades) {
        ...
        for (let x = startOffset; x < f.width - 2; x += winInterval) {
          ...
          for (let ix = sWinX - 1; ix <= sWinX + winW; ix++) {
            ...
            if (
              globalColumnOccupied.has(
                `${f.columns[Math.max(0, Math.min(ix, f.width - 1))].worldX},${f.columns[Math.max(0, Math.min(ix, f.width - 1))].worldZ}`,
              )
            ) {
              canPlace = false;
              break;
      ...
      for (let level = 1; level < numFloors; level++) {
        ...
        for (const slot of groundSlots) {
          ...
          if (Math.random() < 0.2) {
            ...
            if ((type === 'house' || type === 'highrise') && !floorsWithWallAC[level])
              placeAC = true;
          }
          ...
          const checkPlacement = (w: number, h: number) => {
            ...
            for (let ix = sx - 1; ix <= sx + w; ix++) {
              for (let iy = groundYWin - 1; iy <= groundYWin + h; iy++) {
                if (ix < 0 || ix >= f.width || iy < 0 || iy >= f.height || f.occupancy[ix][iy])
                  return false;
              }
            }
            return true;
          ...
          if (canPlaceWin) {
            ...
            for (let ix = sx - 1; ix <= sx + selectedWinW; ix++) {
              for (let iy = groundYWin - 1; iy <= groundYWin + selectedWinH; iy++)
                f.occupancy[ix][iy] = true;
            }
            ...
            const worldX =
              slot.worldPosX !== undefined ? slot.worldPosX : midX + 0.501 * f.normal.dx;
            const worldZ =
              slot.worldPosZ !== undefined ? slot.worldPosZ : midZ + 0.501 * f.normal.dz;
      ...
      if (type === 'factory') {
        ...
        for (let k = 0; k < numRoofObjs; k++) {
          ...
          if (!overlap) {
            ...
            const isNearMapEdge =
              Math.abs(worldX) > halfSize - 6 || Math.abs(worldZ) > halfSize - 6;
  ...
  while (plots.length > 0) {
    ...
    if (splitHoriz) {
      if (p.d < minBlockSize * 2 + baseStreetWidth) {
        if (p.w >= minBlockSize * 2 + baseStreetWidth)
          splitHoriz = false; // Fallback to vertical
        else {
          if (p.w >= minBlockSize && p.d >= minBlockSize) finalPlots.push(p);
          continue;
        }
    ...
    if (!splitHoriz) {
      if (p.w < minBlockSize * 2 + baseStreetWidth) {
        if (p.d >= minBlockSize * 2 + baseStreetWidth)
          splitHoriz = true; // Fallback to horizontal
        else {
          if (p.w >= minBlockSize && p.d >= minBlockSize) finalPlots.push(p);
          continue;
        }
    ...
    if (splitHoriz) {
      ...
      const splitZ =
        minBlockSize + Math.floor(Math.random() * (p.d - baseStreetWidth - minBlockSize * 2 + 1));
      plots.push({ x: p.x, z: p.z, w: p.w, d: splitZ });
      plots.push({
        x: p.x,
        z: p.z + splitZ + baseStreetWidth,
        w: p.w,
        d: p.d - splitZ - baseStreetWidth,
      });
    } else {
      const splitX =
        minBlockSize + Math.floor(Math.random() * (p.w - baseStreetWidth - minBlockSize * 2 + 1));
      plots.push({ x: p.x, z: p.z, w: splitX, d: p.d });
      plots.push({
        x: p.x + splitX + baseStreetWidth,
        z: p.z,
        w: p.w - splitX - baseStreetWidth,
        d: p.d,
      });
    }
  }
  ...
  while (pIndex < finalPlots.length) {
    ...
    if (dec) {
      ...
      if (splitHoriz && plot.d >= minBlockSize * 2 + baseStreetWidth) {
        ...
        finalPlots.push({
          x: plot.x,
          z: plot.z + splitZ + baseStreetWidth,
          w: plot.w,
          d: Math.max(minBlockSize, plot.d - splitZ - baseStreetWidth),
        });
        ...
        finalPlots.push({
          x: plot.x + splitX + baseStreetWidth,
          z: plot.z,
          w: Math.max(minBlockSize, plot.w - splitX - baseStreetWidth),
          d: plot.d,
        });
      }
    }
  ...
  fenceLocations.forEach((loc) => {
    ...
    const neighborCount = [neighbors.n, neighbors.s, neighbors.e, neighbors.w].filter(
      Boolean,
    ).length;
      ...
      for (let dx = 0; dx < 2; dx++) {
        for (let dz = 0; dz < 2; dz++) {
          if (
            ...
            z + dz >= size ||
            tGrid[x + dx][z + dz] !== 0 ||
            isWaterLogic(x + dx - halfSize, z + dz - halfSize)
          ) {
            clusterClear = false;
            break;
          }
  ...
  return {
    ...
    spawnPos,
    ladderZones,
    riverOrientation: hasRiver ? riverOrientation : -1,
    riverFlow: hasRiver ? settings.riverFlow : 0,
    worldSize: size,
  };
};
