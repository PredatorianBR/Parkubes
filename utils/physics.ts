...
// --- CONSTANTS ---
export const GRID_SCALE = 2; // 2 Grid cells per 1 World Unit (0.5m precision)

export const GRAVITY = 30.0;
export const JUMP_FORCE = 9.0;
export const CLIMB_SPEED = 6.0; 
export const MOVE_SPEED_BASE = 8.0;
export const ROLL_SPEED_MULT = 1.3; 
export const CLIMB_THRESHOLD = 0.3; 
export const MAX_CLIMB_HEIGHT = 500.0; 
export const PLAYER_HEIGHT = 1.95;
export const FALL_DAMAGE_HEIGHT = 3.5;
export const PLAYER_RADIUS = 0.4; // Slightly reduced to fit better in 0.5m gaps if needed

// Water Physics
export const WATER_DEPTH_LEVEL = -1.3; 
...
export const isOutOfBounds = (x: number, z: number, halfSize: number) => 
  x < -halfSize - 0.5 || x > halfSize - 0.5 || z < -halfSize - 0.5 || z > halfSize - 0.5;
...
export const updateEntityPhysics = (
    ...
    if (isMoving) {
        ...
        const halfSize = Math.floor(world.size / 2);
        const minBound = -halfSize - 0.5 + PLAYER_RADIUS + 0.01;
        const maxBound = halfSize - 0.5 - PLAYER_RADIUS - 0.01;
        ...
        for (const axis of axes) {
            ...
            const isWaterExit = isInWater && heightDiff > 0 && heightDiff <= 1.5;
